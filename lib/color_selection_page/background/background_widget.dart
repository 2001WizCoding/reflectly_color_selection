...
import 'package:reflectly_color_selection/color_selection_page/background/ripple/ripple_widget.dart';

class WBackground extends StatefulWidget {
  const WBackground({ Key? key, required this.initialColor, required this.child, required this.controller}) : super(key: key);
...
class _WBackgroundState extends State<WBackground> {
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: <Widget>[
        Container(decoration: new BoxDecoration(gradient: _initialGradient())),
...
class BackgroundController extends ChangeNotifier {
  void doPulse(final List<Color> color, final Offset from) {
    ...
    final WRipple ripple = new WRipple(color: color, center: from, onEnd: onEndCallback);
    widgets.add(ripple);
